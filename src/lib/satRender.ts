  ...
  for (let y = 0; y < bufH; y++) {
    ...
    for (let x = 0; x < bufW; x++) {
      ...
      if (land) {
        ...
        // hillshade from terrain heightfield gradient (NW light)
        const e = 0.35 / fTerrain;
        const h1 = fbm(lon, lat, fTerrain, epochSeed + 5);
        const hx = fbm(lon + e, lat, fTerrain, epochSeed + 5) - h1;
        const hy = fbm(lon, lat - e, fTerrain, epochSeed + 5) - h1;
        const shade = 1 + (hx + hy) * 2.2;
        const s = Math.max(0.72, Math.min(1.18, shade));
        c = [c[0] * s, c[1] * s, c[2] * s];
